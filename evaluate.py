    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        for idx in range(batch_score.size(0)):
            ...
            if eval_forward:
                ...
                gold_neighbor_ids = all_triplet_dict.get_neighbors(cur_ex.tail_id, cur_ex.relation,
                                                                   forward=eval_forward)
...
def eval_single_direction(predictor: BertPredictor,
    ...
    topk_scores, topk_indices, metrics, ranks = compute_metrics(hr_tensor=qr_tensor, entities_tensor=entity_tensor,
                                                                target=target, examples=examples,
                                                                batch_size=batch_size, eval_forward=eval_forward)
