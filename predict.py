...
class BertPredictor:
    ...
    def predict_by_examples(self, examples: List[Example]):
        ...
        qr_tensor_list, ae_tensor_list = [], []
        for idx, batch_dict in enumerate(data_loader):
            if self.use_cuda:
                batch_dict = move_to_cuda(batch_dict)
            outputs = self.model(**batch_dict)
            qr_tensor_list.append(outputs['qr_vector'])
            ae_tensor_list.append(outputs['ae_vector'])

        return torch.cat(qr_tensor_list, dim=0), torch.cat(ae_tensor_list, dim=0)
